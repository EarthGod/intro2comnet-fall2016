...
unsigned int getchecksum(tcphead* header, unsigned int srcAddr, unsigned int dstAddr, unsigned short len, char* data){
        unsigned int checksum = 0;
        checksum += (srcAddr >> 16) + (srcAddr & 0xffff);
        checksum += (dstAddr >> 16) + (dstAddr & 0xffff); 
        checksum += IPPROTO_TCP;
        checksum += 0x14;
        checksum += ntohs(header->srcPort) + ntohs(header->destPort);
...
unsigned int getchecksum2(tcphead* thead, unsigned int srcAddr, unsigned int dstAddr, unsigned short len, char* data){
    unsigned int checksum = 0;
    checksum += (srcAddr >> 16) + (srcAddr & 0xffff);
    checksum += (dstAddr >> 16) + (dstAddr & 0xffff);
    checksum += IPPROTO_TCP;
...
int  getSockfd(int sockfd){
    ...
    while(current_tcb && current_p->current){
...
int stud_tcp_input(char *pBuffer, unsigned short len, unsigned int srcAddr, unsigned int dstAddr)
{
    ...
    if(getchecksum(header, ntohl(srcAddr), ntohl(dstAddr), len, NULL) != ntohs(header->checksum))
        return -1;
    int seqAdd = 1;                             
    ...
    switch(current_tcb->state){
        ...
        case ESTABLISHED:
            if(header->flag == PACKET_TYPE_ACK){
                if(len > 20){
                    if(header->ackNo != (current_tcb->seq + len - 20)){
                        tcp_DiscardPkt((char*)header, STUD_TCP_TEST_SEQNO_ERROR);
                        return -1;
                    }
                    ...
                    if(header->ackNo != (current_tcb->seq + 1)){
                        tcp_DiscardPkt((char*)header, STUD_TCP_TEST_SEQNO_ERROR);
                        return -1;
                    }
                    current_tcb->ack=  header->seqNo + 1;
...
void stud_tcp_output(char *pData, unsigned short len, unsigned char flag, unsigned short srcPort, unsigned short dstPort, unsigned int srcAddr, unsigned int dstAddr)
{
    ...
    thead->checksum = ntohs(getchecksum2(thead, srcAddr, dstAddr, len, pData));
    ...
    tcp_sendIpPkt((unsigned char*)thead, len+20, current_tcb->srcAddr, current_tcb->dstAddr, 60);
}
...
int stud_tcp_send(int sockfd, const unsigned char *pData, unsigned short datalen, int flags)
{
    ...
    if(current_tcb->state != ESTABLISHED)
        ...
        uint32_t srcAddr = getIpv4Address();
        uint32_t dstAddr = current_tcb->dstAddr;
...
int stud_tcp_close(int sockfd)
{
    ...
    while(current_p && current_p->current){
        if(current_p->current->sockfd == sockfd){
            current_tcb = current_p->current;
            break;
        }
